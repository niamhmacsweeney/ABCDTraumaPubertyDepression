...
library(kableExtra)
library(ICC) #for calculating ICC for random effects and covariates
...
df <- data %>% 
  select(c(id, src_subject_id, BPM_T4, BPM_6m, trauma_cat, trauma_factor,trauma_dummy, trauma_T1, time, pds_avg, pds_tot, age_years, age_centred, site_id_l, bmi_score, race_ethnicity, education_cat, demo_comb_income_v2, slope, intercept, class, class_label))
...
df <- df %>% 
  filter(!is.na(trauma_cat)) #N = 3668
...
missing_all_vars <- apply(df[, c("id", "BPM_T4", "BPM_6m", "education_cat", "bmi_score")], 1, function(row) all(is.na(row)))

# Extract the IDs of these rows
at_least_one_vars_ids <- df$id[missing_all_vars]

# Print the IDs
print(at_least_one_vars_ids)

# Check if my code in the original script is correct (I wat to keep all rows with at least 1 observation on var1-var5)
## Keep all that have at least one observation
columns_to_checkT <-c("var1","var2","var3","var4","var5")
rows_to_keepT <- apply(data[columns_to_checkT], 1, function(row) any(!is.na(row)))
dataT_Reduced <- data[rows_to_keepT, ]
...
traumaPubertyBarplot
#save as .png
ggsave("../figs/traumaPubertyBarplot.png", plot = traumaPubertyBarplot, width = 16, height = 10, units = "in", dpi = 300)
...
save_kable(html_table, "../figs/trauma_counts_and_percentages_by_class.html")
...
traumaTypeBarChart <- ggplot(frequency_table, aes(x = reorder(Trauma_Category, Count), y = Count, fill = Trauma_Category)) +
  ...
  geom_text(aes(label = Count), hjust = -0.3, size = 8) +  # Add counts on the bars
...
#save as .png
ggsave("../figs/traumaTypeCountsBarChart_new.png", plot = traumaTypeBarChart, width = 26, height = 18, units = "in", dpi = 300)


```

#####4. Intercept and slope variance

```{r}
#make a ridge plot showing variance 

intercept_plot <- df %>% 
  filter(complete.cases(.)) %>% 
                  ggplot(aes(x = intercept, fill = class_label)) +
                  geom_density(alpha = 0.5, adjust = 3 ) + 
                  scale_fill_manual(values = c("#00b9e3", "#ae87ff", "#d39200"),
                    limits = c("Early starters", "Typical developers", "Slow developers"))+
              labs(title = "A: Distribution of individual-level intercept values by puberty class",
              x = "Intercept",
              y = "Density",
              fill = "Class") +
              theme_classic() +
  theme(legend.title = element_blank(),
        legend.text = element_markdown(size = 16, margin = margin(r= 10,l =10)),
...
intercept_plot
#save
ggsave("../figs/intercept_dist.png", intercept_plot, width = 26, height = 26, units = "cm", dpi = 300)
...
slope_plot <- df %>% 
  filter(complete.cases(.)) %>% 
                  ggplot(aes(x = slope, fill = class_label)) +
                  geom_density(alpha = 0.5, adjust = 3 ) + 
                  scale_fill_manual(values = c("#00b9e3", "#ae87ff", "#d39200"),
                    limits = c("Early starters", "Typical developers", "Slow developers"))+
              labs(title = "B: Distribution of individual-level slope values by puberty class",
              x = "Slope",
              y = "Density",
              fill = "Class") +
              theme_classic() +
...
slope_plot
ggsave("../figs/slope_dist.png", slope_plot, width = 26, height = 26, units = "cm", dpi = 300)
...
combined <- ggarrange(intercept_plot, slope_plot, nrow = 2, heights = c(1,1))
combined

#save
ggsave("../figs/intercept_slope_dist.png", combined, width = 26, height = 26, units = "cm", dpi = 300)
...
#remove improbable BMI values that are extreme outliers and will affect correlation matrix
corr_df <- df %>%  
 dplyr::filter(bmi_score > 10) 

corr_df <- df %>% 
  select(c(BPM_T4, BPM_6m, trauma_T1, trauma_dummy, slope, intercept, age_T4, bmi_score))

#rename variable for figure
corr_df <- corr_df %>% 
  rename(`BPM T4` = BPM_T4,
        `BPM 6m` = BPM_6m,
         `Trauma T1 (raw)` = trauma_T1,
        `Trauma T1 (recoded)` = trauma_dummy,
         `Intercept` = intercept, 
         `Slope` = slope,
         `Age T4` = age_T4,
        `BMI` = bmi_score)

#keep complete cases for corr matrix to work 
corr_df <- na.omit(corr_df) #3340 
...
#make it nicer using ggplot
# Visualize with ggcorrplot for publication
corr_plot <- ggcorrplot(cor_matrix, hc.order = TRUE, type = "lower", lab = TRUE,
           lab_size = 6,                           # Increase size of correlation coefficients
           tl.cex = 16,                           # Increase size of variable labels
           tl.col = "black",                       # Label color
           tl.srt = 45,                            # Rotate labels for better readability
           colors = c("#6D9EC1", "white", "#E46726"),  # Diverging color palette
           outline.color = "gray") +                # Add border around the cellS
  ggtitle("Correlation Matrix") +                  # Title of the plot
  theme(
    plot.title = element_text(size = 22, face = "bold", hjust = 0.5),  # Title size and center alignment
    legend.text = element_text(size = 14),                             # Legend text size
    legend.title = element_text(size = 16),                            # Legend title size
    axis.text = element_text(size = 16),                               # Axis label text size
    plot.margin = margin(20, 20, 20, 20)                               # Increase margins
  )
corr_plot

#save as .png
ggsave("../figs/corr_matrix.png", plot = corr_plot, width = 16, height = 12, units = "in", dpi = 300)
...
Check site and ethnicity ICC before including in model as covariates

We will run a linear mixed effects model with these variables as a random effect for each path in our mediation model.
```{r check site and ethnicity ICC}

str(dfScaled)

#trauma to dep (path c)
#random effects from model output
 # site_id_l: VAR= 0.008453 SD=0.09194 
 # race_ethnicity:VAR=0.022515 SD= 0.15005 
modC <- lmer(BPM_T4Trans ~ trauma_cat + age_T4 + (1|site_id_l) + (1|race_ethnicity), data = dfScaled)
summary(modC)
icc(modC) #ICC = 0.031 (combined), race = 0.019; site: 0.007

#trauma to puberty (path a)
# site_id_l   Var = 0.003013 SD =  0.05489 
# race_ethnicity Var = 0.005290 SD = 0.07273
modA <- lmer(intercept ~ trauma_cat + age_T4 + (1|site_id_l) + (1|race_ethnicity), data = dfScaled)
summary(modA) 
icc(modA) #0.028 (combined), race= 0.023 site = 0.017

#trauma to puberty (path b)
# site_id_l   Var = 0.007811 SD =  0.08838  
# race_ethnicity Var = 0.023699 SD = 0.15394
modB <- lmer(BPM_T4Trans ~ pds_avg + age_T4 + (1|site_id_l) + (1|race_ethnicity), data = dfScaled)
summary(modB)
icc(modB)#ICC = 0.032 (combined), site= 0.006, race=0.02
...
# get bias corrected standard errors from the bootstrap
estimates_inter <- parameterestimates(fitInter, boot.ci.type = "bca.simple", standardized = TRUE)

#round to 3 decimal places
estimates_inter[ , sapply(estimates_inter, is.numeric)] <- round(estimates_inter[ , sapply(estimates_inter, is.numeric)], 3)

#export as table
write.csv(estimates_inter, file = "../figs/inter_mediation_output.csv")
...
# get bias corrected standard errors from the bootstrap
estimates_slope <- parameterestimates(fitSlope, boot.ci.type = "bca.simple", standardized = TRUE)

#round to 3 decimal places
estimates_slope[ , sapply(estimates_slope, is.numeric)] <- round(estimates_slope[ , sapply(estimates_slope, is.numeric)], 3)

#export as table
write.csv(estimates_slope, file = "../figs/slope_mediation_output.csv")
...
As we have nominal variables, ethnicity, they will need to be converted to dummy variables. They both have many levels which may cause modelling issues in lavaan. 
...
#However, we need to exclude one dummy variable so we have k-1 dummy varibables

#Make dummy variables 
library(fastDummies)

# Create dummy variable for ethnicity and remove first dummy variable
dfScaled <- dummy_cols(dfScaled, select_columns = "race_ethnicity",remove_first_dummy = TRUE)
...
##BMI 
#There were some extreme outliers in the ABCD sample (i.e., a BMI of 6, which is likely a data error)
#According to the CDC, a BMI less than 10 is well below the normal BMI range and would indicate an data entry error (See data cleaning in prep_descriptive.Rmd script)

#get 1st and 99th percentile to check how extreme these values are
#1st % = BMI of 13, 99th percentile = BMI of 32.6
percentiles <- quantile(dfScaled$BMI, probs = c(0.01, 0.99), na.rm = TRUE)

dfScaled <- dfScaled %>%  
 dplyr::filter(BMI > 10) #should remove 5 ids
...
modelMultiCov <- '
 ...
 race_ ethnicity_Black + race_ethnicity_Hispanic + 
  race_ethnicity_Asian + race_ethnicity_Other 
  + income_num + BMI_std
  
  interc ~ a2*trauma + age + intSxEarly + 
