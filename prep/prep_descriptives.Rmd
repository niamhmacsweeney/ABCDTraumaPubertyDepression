...
Our main and pre-registered analyses focus on females but we have re-ran our main analyses in males, at the request of reviewers. See script prep_descriptives_males.Rmd
...
library(gtsummary)
library(nlme)
library(ggridges) #for rain cloud plots ICC
library(performance) #for calculating 
library(lmerTest)
...
sex_id <- sex_id %>% mutate(sex = recode(sex, 
                                         "1" = "M", 
                                         "2" = "F"))
...
femaleParentPds <- Pds4Wave %>% 
  filter(sex == "F") #obs = 21152
#remove male variables 
femaleParentPds <- femaleParentPds %>% 
  select(-c(pds_m4_p, pds_m5_p))

# Create a custom fill color (green)
custom_fill_color <- "skyblue3"

#make distribution plot 
pdsAvgFemalesLong <- ggplot(data=femaleParentPds, aes(x=pds_avg, group=sex, fill=sex)) +
    geom_density(adjust=2, alpha = 0.5, na.rm = TRUE) +
    facet_wrap(~time) +
labs(
...
pdsAvgFemalesLong
ggsave("../figs/PDSScoresT1toT4.png", pdsAvgFemalesLong, width = 8, height = 6, units = "in", dpi = 300)


#Make raincloud plot for females only 

raincloudPDSFemales <- ggplot(femaleParentPds, aes(x=time, y=pds_avg, fill=sex)) +
  ...
  scale_fill_manual(values=custom_fill_color)
raincloudPDSFemales
ggsave("../figs/PDSRaincloud.png", raincloudPDSFemales, width = 8, height = 6, units = "in", dpi = 300)

```

Make table for parent PDS to check for missing values (females only)
```{r, parent PDS table}
#make table with data and missing data per timepoint 
pdsTable <- femaleParentPds %>% 
  ...
  tbl_summary(
    ...
    label = c(age_years ~ "age",
              ...
              pds_f4_p ~ "Breast development",
              pds_f5b_p ~ "Began menstruation",
              time ~ "time point",
...
```
####Tanner Staging Conversion
At the request of reviewers, we undertook sensitivity analyses to convert the PDS to Tanner Staging for ease of interpretation. 

We followed the scoring procedure used in the Shiftcliff et al. (2009) paper. This code is not publicly available but was acquired from a collaborator who had previously used the code. Note: Shirtcliff et al., 2009 used child self-report PDS. 

PDS = Tanner: (1=1) (2=2) (3=4) (4=5) 
...
```
Get female trauma dataframe only 
...
femalePtsd <- femalePtsd %>%
  mutate(trauma_bin = ifelse(ptsdTot > 0, 1, 0))
# Check it worked
print(femalePtsd)
#convert to factor 
femalePtsd$trauma_bin = as.factor(femalePtsd$trauma_bin)
str(femalePtsd) #check it worked - it did

#check number of participants per group: 0 (no trauma) = 3502; 1 (some trauma exposure) = 2022
trauma_bin_count <- femalePtsd %>% 
  count(trauma_bin)
...
femalePtsd <- femalePtsd %>%
  mutate(trauma_cat = case_when(
        ...
        ptsdTot >= 2 ~ 2))
# Check it worked
summary(femalePtsd$trauma_cat) #check that NAs were handled appropriately --- looks fine! 

#convert to factor 
femalePtsd$trauma_cat = as.factor(femalePtsd$trauma_cat)
str(femalePtsd) #check it worked - it did

#No. of participants per group: 0 (no trauma) = 3502; 1 (1 traumatic event) = 1463; 2 (2 + traumatic events) = 559;
trauma_cat_count <- femalePtsd %>% 
  count(trauma_cat)
...
Plot internalising difficulties scores for males and females to look at spread of data 


```{r, intDiff plot}
#get complete cases for plotting purposes

intDiff3YrPlot <- intDiff3Yr %>% 
filter(complete.cases(.))


# Create the density plot with mean lines and mean annotations
intDiffPlot <- intDiff3YrPlot %>%
  ggplot(aes(x = bpm_y_scr_internal_r, fill = sex, color = sex)) +
  geom_density(alpha = 0.4, adjust = 2.25, show.legend = FALSE) +
  
  # Add vertical lines for the means
  geom_vline(data = intDiff3YrPlot %>% group_by(sex) %>% summarize(mean_bpm = mean(bpm_y_scr_internal_r)),
             aes(xintercept = mean_bpm, color = sex),
             linetype = "dashed", size = 1, show.legend = FALSE) +
  
  # Annotate the mean values
  geom_text(data = intDiff3YrPlot %>% group_by(sex) %>% summarize(mean_bpm = mean(bpm_y_scr_internal_r)),
            aes(x = mean_bpm, y = 0, label = round(mean_bpm, 2)),
            vjust = 0, hjust = 0, size = 6, color = "black", show.legend = FALSE) +
  theme_classic() +
  
  scale_fill_manual(values = c("#f4cc44", "#7fcdbb"), labels = c("Males", "Females")) +
  scale_color_manual(values = c("#f4cc44", "#7fcdbb"), labels = c("Males", "Females")) +
  labs(
    title = "Distribution of internalising difficulties for males and females at T4",
    x = "Internalising difficulties (BPM)",
    y = "Density",
    group = "sex"
  ) +
  ...
  # Add dummy points to create the legend
  geom_point(aes(x = Inf, y = Inf, color = sex), shape = 16, size = 5, show.legend = TRUE) +
...
#save figure
ggsave("../figs/BPM_male_female_means.png", intDiffPlot, width = 14, height = 10, units = "in", dpi = 300)
```


Get female dataframe only 
```{r, intDiff plot}
#make female only df: N = 2955
femaleIntDiff <- intDiff3Yr %>% 
  filter(sex == "F")
summary(femaleIntDiff) #Total N = 4909,  NA =285 
...
merge1 <- left_join(femaleIntDiff, femalePtsd, by = "src_subject_id") #4909
merge2 <- left_join(merge1, femaleParentPds, by = "src_subject_id") 
cleanDf <- left_join(merge2, covs, by = "src_subject_id") 

n_distinct(cleanDf$src_subject_id) #number of observations = 19453, consisting of 4909 unique IDs (this is correct)
...
colnames(cleanDf)
cleanDf <- cleanDf %>% 
  select(-c(age_years.x, age_years.y, eventname.x, eventname.x.x, eventname.y, eventname.y.y, sex.x.x, sex.y, sex.y.y))
...
saveRDS(data, file = "../data/cleanData5.0.rds") #export cleaned dataframe for use in main models 
...
This is code for variable data cleaning that wasn't used in the current analysis but might be useful at a later stage. 

### Youth PDS cleaning and scoring 


```{r score and tidy youth pds}

youthPds <- y_pds %>% 
  select(src_subject_id, sex, eventname, age_years,
         pds_ht2_y, pds_skin2_y, pds_bdyhair_y, pds_m4_y, pds_m5_y, pds_f4_2_y, pds_f5_y)

#change any "I don't know/777" or "Refused to Answer/999" to NA

youthPds[youthPds == 777] <- NA
youthPds[youthPds == 999] <- NA

#rename baseline response so that timepoints are in correct order when plotting 
youthPds$eventname <- str_replace(youthPds$eventname, "baseline_year", "0_baseline_year")
...
#Get average youth PDS score

#use case_when to derive pds_avg using female and male specific items
youthPds <- youthPds %>% 
  mutate(pds_avg_youth = case_when
      (sex == "F" ~ rowMeans(select(youthPds, c(pds_ht2_y, pds_skin2_y, pds_bdyhair_y, pds_f4_2_y, pds_f5_y))),
       sex == "M" ~ rowMeans(select(youthPds, c(pds_ht2_y, pds_skin2_y, pds_bdyhair_y, pds_m4_y, pds_m5_y))),
                             TRUE ~ as.integer(NA)))

#plot
pdsAvgyouthPlot <- ggplot(data=youthPds, aes(x=pds_avg_youth, group=sex, fill=sex)) +
    geom_density(adjust=2, alpha = 0.5, na.rm = TRUE) +
    facet_wrap(~time) +
    theme(panel.spacing = unit(0.1, "lines"),
      axis.ticks.x=element_blank()) +
...
##Make PDS total score
youthPds <- youthPds %>% 
  mutate(pds_tot_youth = case_when
      (sex == "F" ~ rowSums(select(youthPds, c(pds_ht2_y, pds_skin2_y, pds_bdyhair_y, pds_f4_2_y, pds_f5_y))),
       sex == "M" ~ rowSums(select(youthPds, c(pds_ht2_y, pds_skin2_y, pds_bdyhair_y, pds_m4_y, pds_m5_y))),
                             TRUE ~ as.integer(NA)))
#plot
pdsTotyouthPlot <- ggplot(data=youthPds, aes(x=pds_tot_youth, group=sex, fill=sex)) +
    geom_density(adjust=2, alpha = 0.5, na.rm = TRUE) +
    facet_wrap(~time) +
    theme(panel.spacing = unit(0.1, "lines"),
      axis.ticks.x=element_blank()) +
...
#Given the partial follow up data in T5, we will only use follow-up data to T4 as in the analysis with release 4.0 
Pds4WaveYouth <- youthPds %>% #44397 observations
  filter(time != "T5")
summary(Pds4Wave)

#remake table with T1 to T4 - for supplementary materials 
pdsAvgyouthPlot <- ggplot(data=Pds4WaveYouth, aes(x=pds_avg_youth, group=sex, fill=sex)) +
    geom_density(adjust=2, alpha = 0.5, na.rm = TRUE) +
    facet_wrap(~time) +
    theme(panel.spacing = unit(0.1, "lines"),
      axis.ticks.x=element_blank()) +
...
```
```{r make raincloud plot for youth pds change over time}

#use 4 wave follow up data with complete cases for pds_avg and pds_tot
plotPdsDataYouth <- Pds4WaveYouth %>% 
  select(src_subject_id, time, sex, age_years, pds_avg_youth, pds_tot_youth) %>% 
  filter(complete.cases(.))

# Create the raincloud plot
pdsTimeYouth <- plotPdsDataYouth %>%
  ggplot(aes(x = time, y = pds_avg_youth, fill = sex, color = sex), alpha = .6) +
  geom_rain(aes(fill = sex, color = sex, group = sex), alpha = .4, adjust = 2.25, show.legend = FALSE) +  # Map color aesthetic here
  theme_classic() +
  stat_summary(fun = mean, geom = "line", aes(group = sex, color = sex), alpha = 1, size = 1, show.legend = FALSE) +
  stat_summary(fun = mean, geom = "point", aes(group = sex, fill = sex), shape = 1, size = 2, color = "black", alpha = 1, show.legend = FALSE) +
...
#save figure
ggsave("../figs/PDS_male_female_change_youth_report.png", pdsTimeYouth, width = 14, height = 10, units = "in", dpi = 300)

```

Get female sample only 

```{r reduce to females only}

femaleYouthPds <- Pds4WaveYouth %>% 
  filter(sex == "F") #obs = 21152
#remove male variables 
femaleYouthPds <- femaleYouthPds %>% 
  select(-c(pds_m4_y, pds_m5_y))

```


####Correlation between parent and youth report PDS average and total scores


```{r correlation between parent and youth report PDS}

#reduce dataframes to variables needed 

#parent
shortPDSParent <- Pds4Wave %>% 
  select(c("src_subject_id", "sex", "eventname", "age_years", "pds_avg", "pds_tot"))


#youth 
shortPDSYouth <- Pds4WaveYouth %>% 
  select(c("src_subject_id", "sex", "eventname", "age_years", "pds_avg_youth", "pds_tot_youth"))

#merge two short dataframes to make a single dataframe containing youth and parent report PDS

mergedPDS <- left_join(shortPDSParent, shortPDSYouth, by = c("src_subject_id", "sex", "eventname", "age_years"))
#change some variables to factors
mergedPDS$src_subject_id <- as.factor(mergedPDS$src_subject_id)
mergedPDS$eventname <- as.factor(mergedPDS$eventname)
mergedPDS$sex <- as.factor(mergedPDS$sex)


#Due to non-independence of observations of the data and the repeated measures of participants, we will run a mixed-effects model to test the correlation between parent and youth report 

###Full sample: males and females

#basic correlation without accounting for repeated measures
cor.test(mergedPDS$pds_avg, mergedPDS$pds_avg_youth, method = "pearson") #r = 0.77

#calculate correlations for each timepoint
# Function to perform cor.test and extract relevant statistics
get_correlation <- function(x, y) {
  test <- cor.test(x, y, method = "pearson")
  return(data.frame(correlation = test$estimate, p.value = test$p.value))
}

# Group by event name and calculate correlations
results <- mergedPDS %>%
  group_by(eventname) %>%
  summarize(correlation = get_correlation(pds_avg, pds_avg_youth)$correlation,
            p_value = get_correlation(pds_avg, pds_avg_youth)$p.value)

print(results)

# Visualize the data
plotPDSCorr <- mergedPDS %>% 
ggplot(aes(x = pds_tot, y = pds_tot_youth)) +
  geom_point() +
  theme_minimal() +
  labs(title = "Correlation between parent- and youth-reported PDS", x = "PDS Average Parent-report", y = "PDS Average Youth-report")
plotPDSCorr 

# Fit the mixed-effects model
model <- lmer(pds_avg_youth ~ pds_avg*eventname + (1 | src_subject_id), data = mergedPDS)
...
mergedPDSFemales <- mergedPDS %>% 
  filter(sex == "F")

#basic correlation without accounting for repeated measures
cor.test(mergedPDSFemales$pds_avg, mergedPDSFemales$pds_avg_youth, method = "pearson") #r = 0.824

#using correlation function specified above, look at correlations over time for females only 
# Group by event name and calculate correlations
resultsF <- mergedPDSFemales %>%
  group_by(eventname) %>%
  summarize(correlation = get_correlation(pds_avg, pds_avg_youth)$correlation,
            p_value = get_correlation(pds_avg, pds_avg_youth)$p.value)
...
# Fit the mixed-effects model accounting for repeated measures and a time interaction
modelF <- lmer(pds_avg_youth ~ pds_avg + (1 | src_subject_id), data = mergedPDSFemales)
...
mergedPDSMales <- mergedPDS %>% 
  filter(sex == "M")

#basic correlation without accounting for repeated measures
cor.test(mergedPDSMales$pds_avg, mergedPDSMales$pds_avg_youth, method = "spearman") #r = 0.63

#using correlation function specified above, look at correlations over time for males only 
# Group by event name and calculate correlations
resultsM <- mergedPDSMales %>%
  group_by(eventname) %>%
  summarize(correlation = get_correlation(pds_avg, pds_avg_youth)$correlation,
            p_value = get_correlation(pds_avg, pds_avg_youth)$p.value)
print(resultsM)
...
# Fit the mixed-effects model
modelM <- lmer(pds_avg_youth ~ pds_avg + (1 | src_subject_id), data = mergedPDSMales)

# Summarize the model
summary(modelM)

# Calculate ICC
icc <- performance::icc(modelM)
print(icc)
